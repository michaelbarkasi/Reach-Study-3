...
struct Vectors quat1_saved[MaxReadingsToSave];
struct Vectors quat2_saved[MaxReadingsToSave];
...
void setup() {
  ...
  SERIAL_PORT.println();
  SERIAL_PORT.println(F("Program: Faux Sonification"));
...
void loop() {
  ...
  if (MOTION_JUST_ENDED) {
    ...
    if ( IPfound && Realsamplecount > 250 ) { // don't count short accidental jerks under 250ms
      ...
      for ( int i = 0; i < MaxReadingsToSave; i++ ) {
        if ( i <= (Realsamplecount - 1) ) {
          ...
          int amp_reconstructed = (int) faux_amp(i);
          int TF_reconstructed = (int) faux_pitch(i);
      ...
      if ( reachnum == randomsamplenum ) {
        ...
        to_end_start = vdst4(quat1_saved[0],model_q1[MOTsamplecount_max]) + 
                       vdst4(quat1_saved[0],model_q2[MOTsamplecount_max]) + 
                       vdst4(quat1_saved[0],model_q3[MOTsamplecount_max]);
        #else
        to_end_start = vdst4(quat1_saved[0],model_q1[MOTsamplecount_max]) + 
                       vdst4(quat1_saved[0],model_q2[MOTsamplecount_max]);
        #endif
    ...
    // Reset randomized faux sonification parameters
    y1_p = pitchinitial;
    y2_p = random(pitchinitial,(int)(PitchValueMax * 0.33)); // the pitch at the 333th sample
    y3_p = random((int)(PitchValueMax * 0.33),(int)(PitchValueMax * 0.67)); // the pitch at the 667th sample
    y4_p = random((int)(PitchValueMax*0.5),PitchValueMax); // the pitch at the 1000th sample

    y1_a = amp_max;
    y2_a = random(0,(int)(amp_max * 0.67)); // the vol at the 333th sample
    y3_a = random(0,(int)(amp_max * 0.67)); // the vol at the 667th sample
    y4_a = random(0,amp_max); // the vol at the 1000th sample
  ...
  if ( UPDATE_SOUND_NOW && IN_MOTION && IPfound ) {
    ...
    if ( reachnum > randomsamplesize ) {
      ...
      amplitude_update( (int) faux_amp(Realsamplecount) ); 
      TF = (int) ( faux_pitch(Realsamplecount) );
      UpdateTF(TF);
...
float faux_pitch ( int x ) {
  // derived from Lagrange’s Interpolation (four points, at x/sample = 0, 333, 667, 1000)
  float output;
  output = (( ( ( x - 333.0 ) * ( x - 667.0 ) * ( x - 1000 ) ) / ( -1 * 222111000 ) ) * y1_p ) +
           (( ( x * ( x - 667.0 ) * ( x - 1000.0 ) ) / 74185074.0 ) * y2_p) + 
           (( ( x * ( x - 333.0 ) * ( x - 1000.0 ) ) / ( -1 * 74185074.0 ) ) * y3_p) + 
           (( ( x * ( x - 333.0 ) * ( x - 667.0 ) ) / 222111000 ) * y4_p);
  output = constrain(output,PitchValueMin,PitchValueMax);
  return output;
}

float faux_amp ( int x ) {
  // derived from Lagrange’s Interpolation (four points, at x/sample = 0, 333, 667, 1000)
  float output;
  output = (( ( ( x - 333.0 ) * ( x - 667.0 ) * ( x - 1000 ) ) / ( -1 * 222111000 ) ) * y1_a ) +
           (( ( x * ( x - 667.0 ) * ( x - 1000.0 ) ) / 74185074.0 ) * y2_a) + 
           (( ( x * ( x - 333.0 ) * ( x - 1000.0 ) ) / ( -1 * 74185074.0 ) ) * y3_a) + 
           (( ( x * ( x - 333.0 ) * ( x - 667.0 ) ) / 222111000 ) * y4_a);
  output = constrain(output,0,amp_max);
  return output;
}
